...
$('.search-btn').on('click', function(){
    ...
    $('.main-heading').removeClass('text-white').addClass('text-deep-teal-green');
    $('.secondary-heading').removeClass('text-white').addClass('text-deep-teal-green');
...
const cards = $('.cards > div');
    ...
    function createPaginationLinks() {
        ...
        for (let i = 1; i <= totalPages; i++) {
            const pageButton = $('<a></a>')
                .attr('href', '#')
                .addClass('w-8 h-8 flex items-center justify-center rounded-md bg-silver cursor-pointer')
                .text(i)
                .on('click', function(e) {
                    e.preventDefault();
                    currentPage = i;
    ...
    function updatePaginationLinks() {
        ...
        $(`#page-numbers a:contains(${currentPage})`).addClass('active bg-sea-green-deep text-white');
...
$('.getDetailsBtn').on('click',function(){
    ...
    $('.getDetailsBtn').hide();
    $('.uploadPopup > div').addClass('grow-transition').css('max-height', '80vh');
...
$('.cancelBtn').on('click',function(){
    ...
    $('.getDetailsBtn').show();
    $('.uploadPopup > div').removeClass('grow-transition').css('max-height', '45vh');
